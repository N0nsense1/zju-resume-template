...
% --- 第一个项目 ---
\textbf{从零构建的basic编译器} \hfill 2024.05 -- 2024.06 \titlebreak
项目描述：用python语言从零实现的一个编译器引擎，项目完全没有采用任何第三方库，从零实现了简单的basic语言编译器功能。实现了赋值，多级优先级运算，嵌套循环等功能。
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    \item \textbf{标准化编译器组件}: 按标准编译器后端架构实现了Tokenizer(分词器), Parser(语法分析器), Interpreter(解释器)等，与编译器标准后端结构一致。
    \item \textbf{底层算法实现}: 利用简单的分词算法按照basic语法实现分词，利用递归下降算法和抽象语法树构造实现多级优先级计算。运算过程的数据和操作符利用node类高度封装，利于阅读、维护和扩展。
...
\begin{itemize}[nosep]
    \item \textbf{编程语言}: Python(相对熟练), C++, C
    \item \textbf{开发工具}: Git, SSH, Conda, GNUmake, Cmake(掌握基础知识)
    \item \textbf{操作系统}: Linux
    \item \textbf{学习方向(目前)}: 科学计算优化(OpenMP, SIMD, CUDA), AI训练\推理加速(Pytorch，分布式训练)
\end{itemize}
